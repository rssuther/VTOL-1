    ...
    while(True):
        ...
        if frame_delay == 10:
            ...
            if (SHOW_VIS_OUTPUT):
                ...
                cv2.moveWindow('PICAM FRAME '+str(frame_number), 1100, 90)
                #cv2.resizeWindow('PICAM FRAME '+str(frame_number), 1920, 1080)
                cv2.imshow('PICAM FRAME '+str(frame_number), frame)
                cv2.waitKey(1050)
            ...
            if (SHOW_VIS_OUTPUT):
                ...
                cv2.moveWindow('POI ANALYSIS FRAME '+str(frame_number), 1100, 90)
                cv2.imshow('POI ANALYSIS FRAME '+str(frame_number), poi_analysis)
                cv2.waitKey(1050)
...
#def sig_handler_sigint(signum, frame):
 ...
 #   cv2.destroyAllWindows()
#    print('Exiting POI Analysis\n')
 #   exit(1)

def img_poi_detection_demo(show_vis=1):

    #signal.signal(signal.SIGTERM, sig_handler_sigint)

    #show_vis = atoi(show_vis_c)
